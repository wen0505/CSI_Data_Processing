...
2. 將一個 csv 檔案取出子載波，計算其振幅和相位值後，再計算其平均值和標準差，然後儲存到空的 csv 檔案
"""
import csv
import re
...
if __name__ == "__main__":
    ...
    """
    FILE_NAME0 = "C:/PythonProject/plot_csi_figure/src/old/no-people.csv"
    FILE_NAME1 = "C:/PythonProject/plot_csi_figure/src/old/unmanned.csv"

    f0 = open(FILE_NAME0, 'wb+')     # 清空 csv 檔案，參考 https://developer.aliyun.com/article/557213
    f1 = open(FILE_NAME1)

    # with open(FILE_NAME0, "a", newline='', encoding='utf-8') as c:
        # writer = csv.writer(c)                                  # 寫入csv檔
        # writer.writerow(['average', 'standard deviation'])      # 加入標題名稱

    loop1 = 0
    loop = 7000         # 可以選擇要輸入幾行的資料
    ...
    for j1, l1 in enumerate(f1.readlines()):
        ...
        for d in data_sub:
            b = float(d)
            s = s + (b - mean)**2
        sd = sqrt((s/(len(data_sub)-1)))
        # print("標準差 : ", format(sd, '.3f'))
        # print(sd, file=open(FILE_NAME0, 'a'))

        data_zip = [format(mean, '.3f'), format(sd, '.3f')]
        print(data_zip, file=open(FILE_NAME0, 'a'))
